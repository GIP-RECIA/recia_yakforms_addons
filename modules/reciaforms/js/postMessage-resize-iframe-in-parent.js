...
if (!postMessage_resize_iframe_in_parent) {
  ...
  (function () {
    ...
    var getDimen = function (height) {
      ...
      while (queue.length > 0) {
        ...
        if (typeof elem.style !== "undefined" && elem !== document.body) {
          ...
          if (!overflow) {
            overflow = getElementComputedStyle(
              elem,
              height ? "overflowY" : "overflowX"
            );
          }
          ...
          if (typeof child.style != "undefined") {
            ...
            if (typeof start != "undefined" && typeof size != "undefined") {
              ...
              var end =
                start +
                size +
                (parseIntFromElemPxAttribute(child, dimenEnd) || 0);
      ...
      return (
        result +
        (parseIntFromElemPxAttribute(
          document.body,
          height ? "borderBottom" : "borderRight"
        ) || 0) +
        (parseIntFromElemPxAttribute(
          document.body,
          height ? "borderTop" : "borderLeft"
        ) || 0) +
        (parseIntFromElemPxAttribute(
          document.body,
          height ? "marginBottom" : "marginRight"
        ) || 0) +
        (parseIntFromElemPxAttribute(
          document.body,
          height ? "marginTop" : "marginLeft"
        ) || 0) +
        (parseIntFromElemPxAttribute(
          document.body,
          height ? "paddingBottom" : "paddingRight"
        ) || 0) +
        (parseIntFromElemPxAttribute(
          document.body,
    ...
    var postMessageIframeHeight = function (kind) {
      ...
      if (kind !== "load" && !changed) {
        ...
        (height < previousHeight ||
          window.resizeIframe_disableDangerousWindowResize)
      ) {
    ...
    var mayRegisterCheckResize = function () {
      ...
      intervalCount = Math.round(
        ((window.mayRegisterCheckResizeTime || 2) * 1000) / 500
      );
    };
    ...
    if (window.addEventListener) {
      ...
      } else mylog("postMessage-resize-iframe-in-parent: no DOCTYPE, aborting");
    } else {
      // for IE
      ...
      if (ie_has_doctype()) {
        ...
        mylog(
          "postMessage-resize-iframe-in-parent: no DOCTYPE found, aborting"
        );
    }
    ...
    var params =
      window.bandeau_ENT && window.bandeau_ENT.ping_to_increase_session_timeout;
